...
app.controller("LeadController", ["$scope", "CCMAPI", "$controller", "manageLead", "Excel", "$timeout",
    function ($scope, CCMAPI, $controller, manageLead, Excel, $timeout) {
        var self = this;
        $controller('ToastController', {$scope: $scope});
        $controller('DialogController', {$scope: $scope});
...
//        };
        ...
        $scope.expToXl = function(tblId) {
            $scope.exportHref = Excel.tableToExcel(tblId, 'sheet name');
            $timeout(function(){
